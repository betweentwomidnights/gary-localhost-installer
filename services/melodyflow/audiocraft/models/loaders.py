...
def _torch_load_checkpoint(path: str, device='cpu'):
    """Load AudioCraft checkpoints compatibly across torch versions.

    MelodyFlow checkpoints embed OmegaConf objects in `xp.cfg`, so PyTorch 2.6+
    requires those globals to be allowlisted when using `weights_only=True`.
    """
    ...
    if callable(safe_globals):
        with safe_globals(_CHECKPOINT_SAFE_GLOBALS):
            return torch.load(path, map_location=device, weights_only=True)

    if callable(add_safe_globals):
        add_safe_globals(_CHECKPOINT_SAFE_GLOBALS)
...
def _get_state_dict(
    ...
    if os.path.isfile(file_or_url_or_id):
        return _torch_load_checkpoint(file_or_url_or_id, device=device)

    if os.path.isdir(file_or_url_or_id):
        file = f"{file_or_url_or_id}/{filename}"
        return _torch_load_checkpoint(file, device=device)
