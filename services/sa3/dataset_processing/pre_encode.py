...
def _valid_latent_length(actual_samples, padded_samples, latent_length):
    """Map a clip's valid audio length to its encoded latent length."""
    if padded_samples <= 0:
...
def _per_clip_latent_rms(latents, actual_samples, padded_samples):
    """Measure each clip over its valid, non-padded latent region."""
    latent_length = latents.shape[-1]
    measured = []
    for index, valid_samples in enumerate(actual_samples):
        valid_length = _valid_latent_length(
            valid_samples,
            padded_samples,
            latent_length,
        )
        valid = latents[index, ..., :valid_length].float()
        measured.append(valid.pow(2).mean().sqrt().clamp(min=1e-6).item())
    return measured


def encode_with_per_track_norm(
    pretransform,
    audio,
    actual_samples,
    target_latent_rms,
    ...
    """Iteratively normalize each clip to the requested encoded latent RMS."""
    gains = torch.ones(
        audio.shape[0],
        device=audio.device,
        dtype=audio.dtype,
    ...
    for iteration in range(max_iters + 1):
        with torch.no_grad():
            latents = pretransform.model.encode_audio(
                audio * gains.view(-1, 1, 1),
                chunked=chunked,
            )
        measured = _per_clip_latent_rms(
            latents,
            actual_samples,
            audio.shape[-1],
        )
        if pre_norm is None:
            pre_norm = list(measured)

        max_relative_error = max(
            abs(value - target_latent_rms) / target_latent_rms
            for value in measured
        )
        if max_relative_error <= tolerance or iteration == max_iters:
            return latents, gains.tolist(), pre_norm, list(measured), iteration

        corrections = torch.tensor(
            [target_latent_rms / value for value in measured],
            device=audio.device,
            dtype=audio.dtype,
        )
        gains = gains * corrections

    raise RuntimeError("latent RMS normalization did not produce latents")


def _print_norm_summary(prefix, target, tolerance, max_iters, gains, pre, achieved):
    if not achieved:
        return
    worst = max(abs(value - target) / target for value in achieved)
    print(
        f"{prefix}[loudness fix] {len(achieved)} clips, target latent RMS = {target:.4f}"
    )
    print(
        f"{prefix}  pre-norm RMS min={min(pre):.4f} "
        f"mean={statistics.fmean(pre):.4f} "
        f"median={statistics.median(pre):.4f} max={max(pre):.4f}"
    )
    print(
        f"{prefix}  achieved RMS min={min(achieved):.4f} "
        f"mean={statistics.fmean(achieved):.4f} "
        f"median={statistics.median(achieved):.4f} max={max(achieved):.4f} "
        f"std={statistics.pstdev(achieved):.4f}"
    )
    print(
        f"{prefix}  applied gain min={min(gains):.4f} "
        f"mean={statistics.fmean(gains):.4f} max={max(gains):.4f}"
    )
    print(
        f"{prefix}  worst relative error={worst * 100:.2f}% "
        f"(tolerance {tolerance * 100:.0f}%, max correction rounds {max_iters})"
    )
...
def _encode_shard_inner(rank, world_size, cfg):
    ...
    for bi, batch_indices in enumerate(batch_groups):
        ...
        try:
            ...
            actual_sample_counts = [item[3] for item in batch_meta]
            if cfg["per_track_target_latent_rms"] > 0:
                (
                    ...
                    cfg["per_track_target_latent_rms"],
                    cfg["norm_iters"],
                    cfg["norm_tol"],
                    use_chunked,
                )
                print(
                    f"{prefix}  loudness fix settled in "
                    f"{norm_iterations} correction round(s)",
                    flush=True,
                )
            else:
                with torch.no_grad():
                ...
                norm_gains = [1.0] * len(batch_meta)
                pre_norm_rms = [None] * len(batch_meta)
                achieved_rms = [None] * len(batch_meta)
            del audio_batch
            ...
            for bi, (global_idx, fpath, task, actual_samples) in enumerate(batch_meta):
                ...
                try:
                    ...
                    if pre_norm_rms[bi] is not None:
                        meta["audio_gain_applied"] = float(norm_gains[bi])
                        meta["latent_rms_pre_norm"] = float(pre_norm_rms[bi])
                        meta["latent_rms_achieved"] = float(achieved_rms[bi])
                        all_norm_gains.append(float(norm_gains[bi]))
                        all_pre_norm_rms.append(float(pre_norm_rms[bi]))
                        all_achieved_rms.append(float(achieved_rms[bi]))
                    meta.update(tags)
            ...
            for global_idx, fpath, task, actual_samples in batch_meta:
                ...
                try:
                    ...
                    if pre_norm_rms[0] is not None:
                        meta["audio_gain_applied"] = float(norm_gains[0])
                        meta["latent_rms_pre_norm"] = float(pre_norm_rms[0])
                        meta["latent_rms_achieved"] = float(achieved_rms[0])
                        all_norm_gains.append(float(norm_gains[0]))
                        all_pre_norm_rms.append(float(pre_norm_rms[0]))
                        all_achieved_rms.append(float(achieved_rms[0]))
                    meta.update(tags)
                    with open(json_path, "w") as f:
                        json.dump(meta, f)
    ...
    stats = {
        ...
        "skipped": skipped,
        "errors": errors,
        "norm_gains": all_norm_gains,
        "pre_norm_rms": all_pre_norm_rms,
        "achieved_rms": all_achieved_rms,
    }
...
def main():
    ...
    parser.add_argument(
        ...
        dest="per_track_target_latent_rms",
        type=float,
        default=0.0,
        help="If greater than zero, iteratively normalize each clip to this "
             "encoded latent RMS. Recommended loudness-neutral target: 0.90.",
    )
    parser.add_argument(
        "--norm-iters",
        "--norm_iters",
        ...
        type=int,
        default=4,
        help="Maximum correction rounds for latent RMS normalization (default: 4)",
    )
    parser.add_argument(
        "--norm-tol",
        "--norm_tol",
        ...
        type=float,
        default=0.03,
        help="Relative convergence tolerance for latent RMS normalization (default: 0.03)",
    )
    args = parser.parse_args()

    if args.per_track_target_latent_rms < 0:
        parser.error("--per-track-target-latent-rms must be zero or greater")
    if args.norm_iters < 0:
        parser.error("--norm-iters must be zero or greater")
    if not 0 < args.norm_tol < 1:
        parser.error("--norm-tol must be greater than zero and less than one")
    ...
    if args.per_track_target_latent_rms > 0:
        print(
            "  Loudness fix:       enabled "
            f"(target latent RMS {args.per_track_target_latent_rms:.4f})"
        )
    else:
        print("  Loudness fix:       off")
    print()
    ...
    for rank in range(num_gpus):
        ...
        if stats_path.exists():
            ...
            all_norm_gains.extend(s.get("norm_gains", []))
            all_pre_norm_rms.extend(s.get("pre_norm_rms", []))
            all_achieved_rms.extend(s.get("achieved_rms", []))
            stats_path.unlink()
    ...
    if args.per_track_target_latent_rms > 0 and num_gpus > 1:
        _print_norm_summary(
            "",
