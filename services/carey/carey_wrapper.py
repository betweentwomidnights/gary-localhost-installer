        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                while time.time() < deadline:
                    ...
                    print(f"[wrapper] poll: status={ace_status} progress={result.get('progress')} text={_strip_ansi(result.get('progress_text') or '')[:80]}", flush=True)
                    ...
                    if progress_text:
                        ...
                        parsed = _parse_progress_from_text(progress_text)
                        if parsed is not None:
                            job.progress = parsed
                            got_real_progress = True

                    raw_progress = result.get("progress")
                    if isinstance(raw_progress, (int, float)) and raw_progress > 0:
                        job.progress = min(int(raw_progress), 99)
                        got_real_progress = True
